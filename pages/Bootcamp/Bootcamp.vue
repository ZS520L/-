							...
							if (event.action === "join" || event.action === "online") {
								...
								self.currentRoom.onlineUsers.users.forEach((user, index) => {
									if (event.id === user.id) {
										...
										self.currentRoom.messages = []
										self.currentRoom.messages.push(message);
										console.log(offlineUser[0].id,self.currentRoom.onlineUsers.users[0].id)
										...
										// else if(offlineUser[0].id==self.currentRoom.onlineUsers.users[0].id||offlineUser[0].id==self.currentRoom.onlineUsers.users[1].id){
										// 	uni.showModal({
										// 	    title: '提示',
										// 	    content: '玩家退出',
										// 	    success: function (res) {
				...
				pubSub.subscribe({
					...
					onMessage : function (message) {
						...
						if (newMessage.content){
							// self.currentRoom.messages = []
							if (self.currentRoom.messages.length<5){
