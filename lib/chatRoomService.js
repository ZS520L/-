...
function ChatRoomService(roomId, user , messageListener) {
...
ChatRoomService.prototype.initGoeasy = function (user) {
	...
	this.goeasy = GoEasy({
		...
		onConnectFailed : function () {
			console.log("GoEasy连接失败，请确认chatRoomService.js文件87行appkey和host配置正确")
		}
	})
			...
			presenceEvents.events.forEach(function(event) {
				if (event.action == "join" || event.action == "online") {//进入房间
					...
					if(offlineUserIndex>-1) {
						...
						//添加离开消息
						let message = new Message(offlineUser.id, offlineUser.nickname, " 退出房间")
