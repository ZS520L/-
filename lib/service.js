 ...
 */
ChatRoomService.prototype.loadOnlineUsers = function (roomId, loadOnlineUserCallback) {
	this.goeasy.hereNow({
		channels : [roomId],
		includeUsers : true,
		distinct : true
		...
		if(result.code == 200) {
			...
			currentRoomOnlineUsers.users.forEach(function(onlineUser) {
				...
				onlieUsers.users.unshift({
					userId : onlineUser.id,
					nickname :userData.nickname,
					avatar : userData.avatar
				});
			});
			obj.count = currentRoomOnlineUsers.clientAmount;
			loadOnlineUserCallback(onlieUsers)
		}else {
 ...
 */
ChatRoomService.prototype.listenUserOnlineOffline = function (roomId ,userOnlineCallback, userOfflineCallback) {
		...
		onPresence : function (presenceEvents) {
			...
			presenceEvents.events.forEach(function(event) {
				...
				if (event.action == "join" || event.action == "online") {
					...
					userOnlineCallback(obj);
				} else {
					//todo 保存消息  返回上下线后的消息对象  是否在service保存数据？
					userOfflineCallback(event.userId)
				}
			});
		},
		onSuccess : function () {
 ...
 */
ChatRoomService.prototype.listenNewMessage = function (roomId, newMessageCallback, newPropCallback) {
	this.goeasy.subscribe({
		channel: roomId,
		onMessage : function (message) {
			let chatMessage = JSON.parse(message.content);
			if (chatMessage.type == 0) {
				RestApi.uniappSaveChatMessage(this.env, roomId, message);
			}
			if(chatMessage.type == 0) {
				newMessageCallback(chatMessage)
			}
			if(chatMessage.type == 1) {
				newPropCallback(chatMessage)
			}
		},
		onFailed : function() {
			console.log("您还没有高级功能的权限，付费用户请联系GoEasy开通")
...
ChatRoomService.prototype.loadChatMessages = function (roomId) {
	...
	return RestApi.uniappFindChatMessages(this.env, roomId);
};
